...
<script type="text/javascript">
    $(function () {
        if(${MyOrder==null}){
            if(${userInfo!=null}){
                window.location="${pageContext.request.contextPath}/orderservlet/findorder?user_id=${userInfo.user_id}&&yemian=1&&stauts=123";
            }
        }
    });
</script>
    ...
    <div class="user_cont">
         ...
         <div class="user_cont_left">
               <dl>
                   ...
                   <dd class="active"><a href="<%=basePaths%>Bill.jsp">我的账单</a></dd>
                       ...
                       <tr>
                        ...
                        <tr>
                            ...
                            <td colspan="2" style=" text-align:left;padding-left:20px;">
                                 <div class="user_ck"><a href="" >共付<span class="red">${order.get(5)+0.00}</span>元</a></div>
                                 <div class="user_pj"><a href="" >书籍费用<span class="red">${order.get(5)}</span>元</a></div>
                                 <div class="user_sc"><a href="" >运费<span class="red">0.00</span>元</a></div>                            </td>
                        </tr>
                            </c:forEach>
                        </c:if>
                    </table>
                 ...
                 <div style="clear:both;"></div>
                    ...
                    <div class="fenyeys" >
                        ...
                        <c:if test="${MyOrder.get(0).get(6)!=MyOrdernum.get(0).order_id}">
                            <a href="<%=basePath%>orderservlet/findorder?order_id=${MyOrder.get(0).get(6)-1}&&user_id=${userInfo.user_id}&&yemian=1">上一页</a>
                        </c:if>
                        <c:forEach  items="${MyOrdernum}"  var="num"  >
                            <c:if test="${MyOrder.get(0).get(6)==num.order_id}">
                                <a href="<%=basePath%>orderservlet/findorder?order_id=${num.order_id}&&user_id=${userInfo.user_id}&&yemian=1"  class="active"><%=i%></a><%i++;%>
                            </c:if>
                            <c:if test="${MyOrder.get(0).get(6)!=num.order_id}">
                                <a href="<%=basePath%>orderservlet/findorder?order_id=${num.order_id}&&user_id=${userInfo.user_id}&&yemian=1"  ><%=i%></a><%i++;%>
                            </c:if>
                        </c:forEach>
                        <c:if test="${MyOrder.get(0).get(6)!=MyOrdernum.get(MyOrdernum.size()-1).order_id}">
                            <a href="<%=basePath%>orderservlet/findorder?order_id=${MyOrder.get(0).get(6)+1}&&user_id=${userInfo.user_id}&&yemian=1">下一页</a>
                        </c:if>
