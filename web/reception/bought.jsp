...
<title>已经购买的商品</title>
    ...
    <script type="text/javascript">
        $(function () {
            if(${MyOrder==null}){
                if(${userInfo!=null}){
                    window.location="${pageContext.request.contextPath}/orderservlet/findorder?user_id=${userInfo.user_id}&&yemian=2&&status=3";
                }
            }
                ...
                if(confirm('确定要删除所选吗?')){
                    ...
                    $.ajax({
                        ...
                        success: function(msg){
                            if(msg)window.location="${pageContext.request.contextPath}/reception/bought.jsp";
                            else alert("删除失败！！！");
                        }
                    });
                }
    ...
    <div class="user_cont">
         ...
         <div class="user_cont_left">
               <dl>
                   ...
                   <dd class="active"><a href="<%=basePaths%>bought.jsp">已经购买的商品</a></dd>
               </dl>
               <dl>
                   <dt>自助服务</dt>
		...
		$(this).toggleClass("jia"); return false;
         ...
         <div class="user_cont_right">
           <div class="user_lmt">
                    ...
                    <div class="user_lmt2">
                         <div class="user_lmt22"><a href=""><img src="<%=basePaths%>images/user_bt.jpg" width="70" height="22" /></a></div>
                 ...
                 <table width="713" border="0" cellspacing="0" cellpadding="0"  class="my_dd">
                       <tr>
                        ...
                        <tr>
                            ...
                            <td>${order.get(5)}</td>
                            <td>
                                 <div class="user_ck"><a href="${pageContext.request.contextPath}/ShowBookDetail?bookid=${order.get(7)}" >查看</a></div>
                                 ...
                                 <div class="user_sc"><a href="<%=basePath%>orderdetailservlet/deleteorder?order_id=${order.get(6)}&&book_id=${order.get(7)}&&yemian=1" >删除</a></div>
                            </td>
                        </tr>
                            </c:forEach>
                        </c:if>
                   ...
                   <c:if test="${MyOrdernum.size()!=0}">
                       <div class="fenyeys" >
                           ...
                           <c:if test="${MyOrder.get(0).get(6)!=MyOrdernum.get(0).order_id}">
                               <a href="<%=basePath%>orderservlet/findorder?order_id=${MyOrder.get(0).get(6)-1}&&user_id=${userInfo.user_id}&&yemian=2&&status=3">上一页</a>
                           </c:if>
                           <c:forEach  items="${MyOrdernum}"  var="num"  >
                               <c:if test="${MyOrder.get(0).get(6)==num.order_id}">
                                   <a href="<%=basePath%>orderservlet/findorder?order_id=${num.order_id}&&user_id=${userInfo.user_id}&&yemian=2&&status=3"  class="active"><%=i%></a><%i++;%>
                               </c:if>
                               <c:if test="${MyOrder.get(0).get(6)!=num.order_id}">
                                   <a href="<%=basePath%>orderservlet/findorder?order_id=${num.order_id}&&user_id=${userInfo.user_id}&&yemian=2&&status=3"  ><%=i%></a><%i++;%>
                               </c:if>
                           </c:forEach>
                           <c:if test="${MyOrder.get(0).get(6)!=MyOrdernum.get(MyOrdernum.size()-1).order_id}">
                               <a href="<%=basePath%>orderservlet/findorder?order_id=${MyOrder.get(0).get(6)+1}&&user_id=${userInfo.user_id}&&yemian=2&&status=3">下一页</a>
                           </c:if>
